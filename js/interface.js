...
$(document).ready(function() {
	...
	$('#toolbarHolder').mouseover(function() {
		if (_contextMenu) _contextMenu.hide();
	});
	...
	$('.ui-layout-resizer').bind('selectstart', function() {
		return false;
	});
	...
	$("#object-filter-text").quickText().bind('keyup', function() {
		...
		timeoutId = setTimeout(function(val) {
			$("#object_list").jstree(true).search(val);
		}, 500, $(this).val());
	});
...
function switchEditor(n) {
	$(".ui-layout-data-south").tabs("option", "active", n);
}
