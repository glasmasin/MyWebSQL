...
class ObjTree
{
    ...
    private $objectTypes = array(
        array('name' => 'table', 'class' => 'tablef', 'groupby' => '/^tbl(.+?)_/'),
        array('name' => 'view', 'class' => 'viewf', 'groupby' => '/^v_(.+?)(_|$)/'),
        array('name' => 'procedure', 'class' => 'procf', 'groupby' => '/^p_(.+?)_/'),
        array('name' => 'function', 'class' => 'funcf', 'groupby' => '/^(f_.+?|_sys|[a-z]{3,})[^a-z]/'),
        array('name' => 'trigger', 'class' => 'trigf', 'groupby' => '/(.+?)_/'),
        array('name' => 'event', 'class' => 'evtf', 'groupby' => false),
        );
    ...
    protected function processObjectType($objectType, $schema) {
        ...
        foreach($objects as $object) {
            ...
            $nodes[] = array(
                ...
                "text" => $object,
                'icon' => "jstree-file",
                "a_attr" => array("href" => 'javascript:objDefault(\'' . $objectType['name'] . '\', \''.$id.'\', \''.$schema_id.'\')')
            );
...
print '</div><script>var jsTreeData = ' . $objTree->getData() . '; </script><div>';
//print '</ul>';
?>